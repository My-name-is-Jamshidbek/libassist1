...
def superadmin_messages(request):
    ...
    return render(request, 'admin/admin_habarlar.html', context)
...
def superadmin_settings(request):
    ...
    return render(request, 'admin/admin_sozlamalar.html', context)


def all_book(request):
    if request.method == 'POST':
        search = request.POST['search']
        book = Book.objects.filter(book_name__contains=search)[:100]
        context = {
        'book': book,
        }
...
def talaba_tanla_admin(request):
    ...
    return render(request, 'admin/talaba_select.html', {'group': Group.objects.all(),'member':Students.objects.all()})
